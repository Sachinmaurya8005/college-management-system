...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const { students, teachers, courses, fees, notices, attendanceSessions } = useCollegeData();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Class Teachers Live Student Attendance Audit Monitor */}
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-card space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 text-xs font-bold border border-blue-200 dark:border-blue-800 mb-1.5">
              <ShieldCheck className="w-3.5 h-3.5" />
              <span>Institutional Attendance Oversight • कक्षा अध्यापकों द्वारा दर्ज हाजिरी की लाइव निगरानी</span>
            </div>
            <h3 className="text-lg font-extrabold text-slate-900 dark:text-white tracking-tight">
              Class Teachers' Student Attendance Records (अध्यापकों द्वारा ली गई हाजिरी)
            </h3>
            <p className="text-xs text-slate-500">
              Live audit of daily student attendance submitted by designated class teachers across all engineering departments.
            </p>
          </div>
          <button
            onClick={() => onNavigate('attendance')}
            className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shadow-md flex items-center gap-1.5 self-start sm:self-center transition-all"
          >
            <CheckSquare className="w-4 h-4" /> Open Full Attendance Register
          </button>
        </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-slate-800/70 font-bold text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-800">
              <tr>
                <th className="py-3 px-4">Date</th>
                <th className="py-3 px-4">Class &amp; Semester</th>
                <th className="py-3 px-4">Subject / Topic</th>
                <th className="py-3 px-4">Class Teacher / Marked By</th>
                <th className="py-3 px-4 text-center">Present / Absent</th>
                <th className="py-3 px-4 text-center">Att. %</th>
                <th className="py-3 px-4 text-right">Audit &amp; Roster</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {(attendanceSessions || []).slice(0, 6).map((session, idx) => (
                <tr key={session.id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-4 font-bold text-blue-600 dark:text-blue-400">
                    {session.branch} (Sem-{session.semester})
                  </td>
                  <td className="py-3 px-4 text-slate-700 dark:text-slate-300">
                    {session.subject}
                  </td>
                  <td className="py-3 px-4">
                    <span className="font-semibold text-slate-900 dark:text-white">{session.markedBy}</span>
                  </td>
                  <td className="py-3 px-4 text-center">
                    <span className="font-bold text-emerald-600">{session.presentCount} P</span>
                    <span className="text-slate-400 mx-1">/</span>
                    <span className="font-bold text-rose-600">{session.absentCount} A</span>
                  </td>
                  <td className="py-3 px-4 text-center">
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 hover:bg-blue-100 text-xs font-bold inline-flex items-center gap-1 transition-all"
                    >
                      <Eye className="w-3.5 h-3.5" /> View Roll-Call
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {/* Modal: View Detailed Student Roll-Call for Session */}
      {selectedSessionForModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-2xl overflow-hidden animate-scale-up">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div>
                <span className="px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-800 text-[10px] font-bold uppercase">
                  Class Teacher Attendance Roster
                </span>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mt-1">
                  {selectedSessionForModal.branch} • Semester {selectedSessionForModal.semester}
                </h3>
                <p className="text-xs text-slate-500">
                  Date: <strong>{selectedSessionForModal.date}</strong> • Subject: <strong>{selectedSessionForModal.subject}</strong> • Teacher: <strong>{selectedSessionForModal.markedBy}</strong>
                </p>
              </div>
              <button
            ...
            <div className="p-6 max-h-96 overflow-y-auto space-y-2">
              {(selectedSessionForModal.records || []).map((record, index) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">{record.studentName}</span>
                      <span className="font-mono text-[10px] text-slate-500">Roll: {record.rollNo}</span>
                    </div>
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 flex items-center justify-between text-xs">
              <span className="text-slate-600 dark:text-slate-400 font-semibold">
                Summary: {selectedSessionForModal.presentCount} Present • {selectedSessionForModal.absentCount} Absent ({selectedSessionForModal.percentage}%)
              </span>
              <button
                onClick={() => setSelectedSessionForModal(null)}
